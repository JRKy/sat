...
let satellites = [];
let satMarkers = new Map(); // name -> Leaflet marker
let lines = [];
...
function refreshMarkerSelection() {
  for (const sat of satellites) {
    ...
    if (!marker) continue;
    marker.setIcon(satIcon(selectedSatName === sat.name));
    if (selectedSatName === sat.name) marker.bringToFront();
  }
}
...
function buildTable(rows) {
  ...
  satTable.innerHTML = `<table>${thead}${tbody}</table>`;
...
function updateLocation(lat, lon, heightKm = 0, setZoom = false) {
  ...
  filtered.forEach(r => {
    if (r.el <= 0) return; // behind Earth → no line in 2D
  ...
  // ✅ Restore useful bottom-left lat/lon always
  info.innerHTML = `
...
// Auto-detect location on load (best effort)
